package com.amap.map2d.demo.busline;
...
public class BuslineActivity extends Activity implements OnMarkerClickListener,
		InfoWindowAdapter, OnItemSelectedListener, OnBusLineSearchListener, OnClickListener {
	private AMap aMap;
	private MapView mapView;
	private ProgressDialog progDialog = null;// 进度框
	private EditText searchName;// 输入公交线路名称
	private Spinner selectCity;// 选择城市下拉列表
	private String[] itemCitys = { "北京-010", "广州市-020", "上海-021" };
			...
			public void onListItemClick(BusLineDialog dialog,
				...
				for (BusLineItem busLineItem : busLineItems) {
					if (busLineItem.getBusLineId().equals(lineId)){
						Log.d("BuslineActivity","选中的公交的所有站点信息=公交线路======" + busLineItem.getBusLineId());
						for (BusStationItem busStationItem : busLineItem.getBusStations()) {
							Log.d("BuslineActivity","选中的公交的所有站点信息=======" + busStationItem.getBusStationName());
							Log.d("BuslineActivity","选中的公交的所有站点信息=======" + busStationItem.getLatLonPoint());
						}
					}
